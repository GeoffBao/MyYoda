...
- `BrowserNavigate`：导航到任意 Chromium 可加载的 URL（不限协议、不限目标网络）；无协议或协议相对输入默认按 HTTPS 处理，`localhost`/`127.0.0.1`/`::1`/`*.localhost` 等本机开发服务自动按 HTTP 打开。
...
- 受管浏览器不再按目标网络、协议或 URL 认证信息限制导航（包括私网地址），具体能否加载由 Chromium 决定；仍会阻止下载、弹窗与网页权限请求，不要尝试规避这些边界。
