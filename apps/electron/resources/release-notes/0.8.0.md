...
### Agent Runtime 统一为 Pi Agent SDK

废弃 Claude Code CLI runtime，统一使用 Pi Agent SDK 作为唯一 Agent 执行引擎，消除了双 runtime 维护成本、渠道兼容差异与权限行为不一致。所有 Claude 渠道流量自动走 Pi adapter，存量渠道 `baseUrl` 配置自动兜底。

### Workspace / Project 术语整合
...
- **文件面板快捷键**：支持在预览 Tab 中切换文件面板显示
- **排队消息可中断**：Agent 排队中的待发送消息支持取消
- **委托子会话清理**：已完成/失败的委托子会话不再显示为未读完成标记
- **文件暴露扩展**：Agent 可见文件范围超出会话 attachments，支持 Project 内更广泛的文件访问

## 跨平台修复

- **Windows 窗口控制与拖拽修复**
- **Mac 构建失败兜底**：重试耗尽后正确报错而非无限等待
