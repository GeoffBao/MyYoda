...
function createXaiRuntimeCredentialStore(
  ...
  return {
    ...
    async modify(
      ...
      const nextCredential: XaiRuntimeCredential = { type: 'oauth', ...refreshed }
      credential = nextCredential
      if (
        previous.access !== nextCredential.access
        || previous.refresh !== nextCredential.refresh
        || previous.expires !== nextCredential.expires
      ) {
        try {
          await onRefreshed?.(nextCredential)
        } catch (error) {
...
async function findPiCatalogModel(provider: ProviderType, modelId: string): Promise<PiCatalogModel | undefined> {
  ...
  const fallbackProviders = getProviders().filter((candidate) => !preferredProviders.includes(candidate as never))
