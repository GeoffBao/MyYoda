...
import { PLANNING_CONFLICT_ERROR } from '@myyoda/shared'
import type {
...
type TodoRow = {
  ...
  due_at: number | null; group_id: string | null; workspace_id: string | null; project_id: string | null; native_connection_id: string | null
...
type CalendarEventRow = {
  ...
  calendar_group_id: string | null; workspace_id: string | null; project_id: string | null; todo_id: string | null; native_connection_id: string | null
...
function migrateDatabase(db: SqliteDatabase): void {
  ...
  if (version > PLANNING_SCHEMA_VERSION) throw new Error('任务/日程数据版本高于当前 MyYoda，无法安全打开')
  ...
  try {
    if (version < 1) {
      ...
      `)
      // MyYoda 扩展列 project_id（Todo/日程关联 craft Project）；已存在的旧库
...
function hydrateCalendarEvents(rows: CalendarEventRow[]): CalendarEvent[] {
  ...
  return rows.map((row) => ({
    ...
    projectId: row.project_id ?? undefined, todoId: row.todo_id ?? undefined, nativeOrigin: nativeOrigin(row.native_connection_id), createdAt: row.created_at, updatedAt: row.updated_at,
  }))
}
function todoFromRow(row: TodoRow): Todo {
  return { id: row.id, title: row.title, notes: row.notes ?? undefined, status: row.status, priority: row.priority, dueAt: row.due_at ?? undefined, groupId: row.group_id ?? undefined, group: getPlanningGroup(row.group_id, 'todo'), tags: getTags('todo', row.id), reminders: getReminders('todo', row.id), sessionLinks: getTodoSessionLinks(row.id), workspaceId: row.workspace_id ?? undefined, projectId: row.project_id ?? undefined, nativeOrigin: nativeOrigin(row.native_connection_id), createdAt: row.created_at, updatedAt: row.updated_at, completedAt: row.completed_at ?? undefined }
}
function calendarEventFromRow(row: CalendarEventRow): CalendarEvent {
  return { id: row.id, title: row.title, notes: row.notes ?? undefined, startAt: row.start_at, endAt: row.end_at ?? undefined, allDay: row.all_day === 1, groupId: row.calendar_group_id ?? undefined, group: getPlanningGroup(row.calendar_group_id, 'calendar'), tags: getTags('calendar_event', row.id), reminders: getReminders('calendar_event', row.id), workspaceId: row.workspace_id ?? undefined, projectId: row.project_id ?? undefined, todoId: row.todo_id ?? undefined, nativeOrigin: nativeOrigin(row.native_connection_id), createdAt: row.created_at, updatedAt: row.updated_at }
}
...
export function connectPlanningNativeConnection(input: ConnectPlanningNativeConnectionInput): PlanningNativeConnection {
  ...
  if (getDatabase().prepare('SELECT id FROM planning_sync_profiles WHERE entity=:entity AND target_id=:targetId').get({ entity: input.entity, targetId: target.id })) throw new Error('MyYoda 受管目标不能同时作为外部连接')
...
/** 断开只移除 MyYoda 投影与映射，绝不删除用户的系统原始事项。 */
...
/** 冲突必须显式选择；保留系统会回流，保留 MyYoda 才会继续/重建其出站版本。 */
...
export function applyPlanningNativeConnectionItems(connectionId: string, items: PlanningNativeExternalItem[], options: { fullSnapshot?: boolean } = {}): void {
  ...
  withPlanningTransaction(() => {
    ...
    // 系统端删除后只移除 MyYoda 投影和 binding；绝不通过 outbox 删除 EventKit 原项。
    const bindings = getDatabase().prepare('SELECT * FROM planning_native_bindings WHERE connection_id=:connectionId').all({ connectionId }) as NativeBindingRow[]
    for (const binding of bindings) {
...
/**
 * MyYoda 受管 Calendar 的回流路径。直接写 SQLite，绝不调用正常 update/enqueue，
