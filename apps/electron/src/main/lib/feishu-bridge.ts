...
import { FEISHU_IPC_CHANNELS, AGENT_IPC_CHANNELS } from '@myyoda/shared'
...
class FeishuBridge {
  ...
  stop(): void {
    ...
    // 重启后仍需用来给桌面 Session 镜像建群。完整重置请删除 ~/.myyoda/feishu-metadata-{botId}.json。
    this.botOpenId = null
  ...
  /**
   * 为 MyYoda 桌面端会话准备飞书镜像群。
   *
   * 该群只包含用户与当前 Bot。用户在群里继续发送消息时，会通过
   * source=session-mirror 的绑定回到同一个 MyYoda session。
   */
  ...
  private async createNewSession(
    ...
    if (!channelId) {
      await this.sendMessage(chatId, '请先在 MyYoda Agent 设置中选择渠道。')
      return
    }
  ...
  private async handleModelCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    if (channels.length === 0) {
      await this.sendMessage(
        chatId,
        '暂无可用渠道。请先在 MyYoda 设置中配置并启用渠道（需填入 API Key 且至少启用一个模型）。',
      )
      return
    }
    ...
    if (!targetBinding) {
      ...
      if (!targetBinding) {
        await this.sendMessage(chatId, '请先发送一条消息创建会话，或在 MyYoda 设置中选择 Agent 渠道。')
        return
      }
    }
  ...
  private handleAgentPayload(sessionId: string, payload: AgentStreamPayload): void {
    ...
    if (payload.kind === 'myyoda_event' && payload.event.type === 'title_updated') {
