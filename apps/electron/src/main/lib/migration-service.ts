 ...
 *
 * 支持两种导出模式：
 * - personal (.myyoda-backup)：个人全量备份，含解密后的 API Key 明文
 * - share (.myyoda-share)：团队分发，自由选择组件，凭据自动剥离
 *
...
export async function parseImportFile(filePath: string): Promise<ImportPreview | ImportPreviewV2> {
  const tempDir = join(tmpdir(), `myyoda-import-${randomUUID()}`)
 ...
 *
 * 品牌演进历史：~/.proma → ~/.luxcoder → ~/.myyoda
 *
 * 迁移链（每级只迁移一次，写入独立 flag，避免重复执行）：
 * 1. 正式版：~/.luxcoder → ~/.myyoda（flag: .migrated-from-luxcoder）
 *    dev 模式：~/.luxcoder-dev → ~/.myyoda-dev
 * 2. 兼容老用户：仅当不存在 ~/.luxcoder 时才从 ~/.proma 迁移
 *    （flag: .migrated-from-proma，注意这是 LuxCoder 时代的旧 flag，
 *     迁移到 myyoda 后应写入新 flag .migrated-from-luxcoder 以保持一致）
 *
 * 非破坏性迁移，保留原目录。迁移完成后写入 flag 文件，避免重复执行。
 */
export function migrateDataDirIfNeeded(): void {
  const home = homedir()
  // 复用 config-paths.ts 的权威 dev/prod 判定（含 MYYODA_DEV=1 覆盖），
  // 避免在此处重复实现 app.isPackaged 判断导致两处逻辑不一致。
  const isDev = getConfigDirName() === '.myyoda-dev'

  if (!isDev) {
    // 正式版目录迁移链：优先 ~/.luxcoder → ~/.myyoda；老用户回退 ~/.proma → ~/.myyoda
    if (existsSync(join(home, '.luxcoder'))) {
      migrateDirChain(join(home, '.luxcoder'), join(home, '.myyoda'), '.migrated-from-luxcoder')
    } else {
      migrateDirChain(join(home, '.proma'), join(home, '.myyoda'), '.migrated-from-proma')
    }
  } else {
    // dev 模式目录迁移链：优先 ~/.luxcoder-dev → ~/.myyoda-dev；老用户回退 ~/.proma-dev → ~/.myyoda-dev
    if (existsSync(join(home, '.luxcoder-dev'))) {
      migrateDirChain(join(home, '.luxcoder-dev'), join(home, '.myyoda-dev'), '.migrated-from-luxcoder')
    } else {
      migrateDirChain(join(home, '.proma-dev'), join(home, '.myyoda-dev'), '.migrated-from-proma')
    }
  }
}
