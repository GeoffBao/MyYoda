...
const agentStreamingStateStorageAtomFamily = atomFamily((_sessionId: string) =>
...
export const agentSidePanelOpenAtom = atomWithStorage<boolean>('myyoda-agent-sidepanel-open', true)
...
export function applyAgentEvent(
  ...
  switch (event.type) {
    ...
    case 'complete': {
      ...
      // 压缩后预估状态（contextUsageIsEstimated）下，即使 result.usage 全 0（如压缩收尾的
      // 合成 result）也不能覆盖预估值；只有拿到真正 > 0 的 inputTokens 才清除预估状态。
