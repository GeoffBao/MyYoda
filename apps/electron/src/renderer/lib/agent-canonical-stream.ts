...
function projectMyYodaEvent(payload: Extract<AgentRendererStreamPayload, { kind: 'myyoda_event' }>): AgentLiveUpdate[] {
...
export function projectAgentLiveUpdates(payload: AgentRendererStreamPayload): AgentLiveUpdate[] {
  ...
  if (payload.kind === 'myyoda_event') return projectMyYodaEvent(payload)
