...
import type { PlanningGroup, Todo } from '@myyoda/shared'
...
function TodoWorkspace({ standalone = false }: { standalone?: boolean } = {}): React.ReactElement {
  ...
  // 隐藏容器 Project（home/ad-hoc）只用于看板卡片归属展示，不出现在「关联项目」选择器里。
  ...
  const startTodoInWorkspace = React.useCallback(async (todo: Todo, workspaceId: string): Promise<void> => {
    ...
    try {
      ...
      toast.error(message.includes(PLANNING_CONFLICT_ERROR) ? 'Todo 已在其他窗口更新，请确认工作区后再启动' : '启动 Agent 失败', {
