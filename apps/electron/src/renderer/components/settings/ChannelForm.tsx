...
import { normalizeBaseUrl, resolveAnthropicMessagesUrl, resolveOpenAIChatCompletionsUrl, resolveOpenAIResponsesUrl } from '@myyoda/core'
...
function isThirdPartyBaseUrl(provider: ProviderType, baseUrl: string): boolean {
  ...
  // provider 官方默认地址为空说明该供应商本就要求用户自填任意端点（如 custom / anthropic-compatible），
  // 不存在"官方默认"可比较，不应一律判定为第三方中转风险。
  if (!defaultBaseUrl) return false
  const normalizedBaseUrl = normalizeBaseUrl(baseUrl)
  return Boolean(normalizedBaseUrl) && normalizedBaseUrl !== normalizeBaseUrl(defaultBaseUrl)
}
...
export function ChannelForm({ channel, onSaved, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  const isSubscriptionProvider = isCodexProvider || isClaudeOAuthProvider || isXaiProvider
  ...
  /** 取消进行中的 ChatGPT OAuth 登录，避免卡在"等待授权完成…"无法恢复 */
      ...
      <SettingsSection title="基本信息">
        <SettingsCard>
          ...
          <SettingsInput
            ...
            required
          />
          {/* 订阅 provider 的请求地址由官方二进制/SDK 内置管理，无需用户填写 */}
          ...
          <div className="px-4 py-3 space-y-2">
            ...
            {isCodexProvider ? (
              <div className="space-y-2">
                ...
                {codexLoggingIn && (
                  <Button variant="ghost" size="sm" type="button" onClick={handleCodexOAuthCancel} className="w-full text-muted-foreground">
                    取消登录
                  </Button>
                )}
                {codexDeviceCode && (
                ...
                ) : <div className="text-xs text-muted-foreground">MyYoda 会代理 token 请求；系统浏览器授权页仍需使用可访问 OpenAI 的网络。可改用设备码并在另一台设备完成授权。</div>}
              </div>
            ) : isXaiProvider ? (
              <div className="space-y-2">
                <Button
                ...
                ) : <div className="text-xs text-muted-foreground">MyYoda 会代理设备码与 token 请求；授权页由系统浏览器打开。若浏览器无可用代理，可扫码在另一台设备完成授权。</div>}
              </div>
            ) : isClaudeOAuthProvider ? (
              <div className="space-y-2">
                <Button
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2">
                ...
                <p>MyYoda 仅作为本地 Agent 执行环境：配置、会话等本地数据均存储在你的设备上，MyYoda 本身不会额外构成数据风险。</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
