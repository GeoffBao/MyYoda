  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: '工具由连接器（全局 MCP、API Key 工具、自定义 HTTP）、Skills（全局 + 工作区 + 项目叠加）和总览里的内置能力共同决定。社区市场提供 60+ 可安装 Skill，覆盖文档、视频、代码审查等场景。会话执行时右侧面板实时显示工具活动；遇到需要确认的操作，MyYoda 会先请求你的授权。',
        keywords: ['连接器', 'MCP', '权限', '技能', '社区市场'],
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: '两者范围不同，不是「统一工作区级」。MCP 在全局 ~/.myyoda/mcp.json，所有工作区共享；只有某个 Project 自己写过 MCP 时才整份覆盖全局，且仅该项目生效。Skills 是全局 + 工作区 + 项目三层叠加。专家、Chat 工具和内置 MCP 是全局。Memory 是工作区级。插件中心顶部同时有「工作区切换」和独立的「作用域选择器（默认配置 / 某 Project）」；默认配置看的是全局 MCP + 当前工作区 Skills，不等于「全部项目共享」。',
        keywords: ['工作区', '作用域', '切换器', 'MCP', '连接器', '默认配置'],
      },
      {
        question: '插件中心顶部的「默认配置」和某个 Project 有什么区别？',
        answer: '工作区切换和作用域选择器是两套东西。选「默认配置」时，连接器展示全局 MCP，技能展示全局 + 当前工作区叠加。切到某个 Project 后：未自建 MCP 仍显示全局 MCP，不会因此「看不到默认插件」；该项目自己配置过 MCP 才整份覆盖、仅本项目生效。Skills 始终继续叠加，项目层只是多一层。',
        keywords: ['默认配置', 'Project', '作用域选择器', 'MCP', '技能'],
      },
      {
        question: '插件中心里的连接器和 MCP/API 是什么关系？',
        answer: '连接器是面向用户的外部系统/工具接入入口。底层可能是 MCP Server、API Key 工具或自定义 HTTP 工具。普通用户只需要在「插件 → 连接器」里配置和启用；高级用户仍可在详情里看到底层类型。定时任务、协作子 Agent、创建任务和 Planning 在总览的「内置能力」里，不是连接器。Brave Search MCP 与 Tavily 联网搜索都属于连接器，凭据不能混用。',
        keywords: ['连接器', 'MCP', 'API', 'Tavily', 'Brave'],
      },
      {
