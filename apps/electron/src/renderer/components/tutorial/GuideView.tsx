...
const GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    steps: ['模型不可用：检查渠道、模型和网络代理', 'Agent 不执行：检查权限模式与工具是否启用', '找不到文件：检查项目（工作区）绑定的工程目录；未绑定项目时可配置「设置 → 工作区 → 默认工作区目录」', '项目目录失效：报错卡片提供「重新关联目录」「设置默认工作区目录」，并自动探测移动候选（如 LuxAgents → LuxAgent）一键关联', '任务失败：查看运行记录后从对应会话继续'],
    example: 'Agent 没有修改文件时，先查看工具活动和权限提示，再确认当前会话绑定的 Project 与工作目录。',
  },
  {
    id: 'discover',
    eyebrow: 'Learn & connect',
    title: '通过「发现」跟进官方内容，参与社区讨论',
    summary: '左侧栏「发现」（与「功能」并列）聚合官方精选、社区讨论、帮助和反馈：看视频学用法，浏览社区经验，翻查指南 FAQ，遇到问题直接反馈。',
    steps: ['官方精选：视频直接在线播放（失败可下载后播放），教程和公告直接阅读，外链跳浏览器', '看到「更新」标记说明有新内容或新版本，点开即已读，入口徽标随之消失', '社区讨论按问题讨论 / 经验分享 / 公告三个板块浏览，「回复」「发起讨论」会跳转 GitHub', '「帮助」分区集中了使用指南、FAQ 和键盘快捷键三个入口；网络不可用时内容自动降级为离线缓存'],
    example: '打开「发现 → 官方精选」，在线播放宣传片与功能指南；到「帮助」翻查使用指南；有疑问在浏览器里发起讨论或提交反馈。',
  },
] as const
