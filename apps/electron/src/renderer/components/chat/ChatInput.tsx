 ...
 * - 监听 myyoda:clear-context 和 myyoda:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('myyoda:clear-context', handler)
    return () => window.removeEventListener('myyoda:clear-context', handler)
  }, [onClearContext])
  ...
  return (
    <div className="px-2.5 pb-2.5 md:px-[18px] md:pb-[18px]" data-input-mode="chat">
        ...
        <div
          className={cn(
            'rounded-[17px] border-[0.5px] border-border bg-background/95 transition-[border-color,box-shadow,background-color] duration-base ease-out',
