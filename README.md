...
- 左侧栏「发现」（与「功能」并列）：官方精选 + 社区讨论 + 帮助 + 反馈四合一入口；
- 官方内容（视频 / 教程 / 公告 / 外链）来自公开内容仓库，带版本号与「更新」红点；
- 视频在线流式播放（代理感知转发），失败可下载后播放，支持版本更新与本地缓存；
- 社区讨论基于 GitHub Discussions 只读浏览，发帖/回复跳浏览器；
- 帮助分区收纳使用指南、FAQ 与键盘快捷键入口；
- 网络不可用时自动降级：缓存优先展示 + 离线横幅，无缓存时给出错误态与重试；
- 反馈复用 Notion 提交通道，与设置中的入口一致。
